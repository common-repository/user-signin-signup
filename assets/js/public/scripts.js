jQuery(document).ready(function ($) {         
    ...
    //SHOW HIDE PASSWORD FIELD for register page
    const iusisu_user_reg = document.querySelector('#iusisu-user-reg');
    if(iusisu_user_reg){
    ...
    // SHOW HIDE PASSWORD FIELD for login page
    const iusisu_login = document.querySelector("#iusisu_login");
    if(iusisu_login){ 
        const iusisu_togglePassword = document.querySelector('#iusisu_login #iusisu_togglePassword');
        const iusisu_password = document.querySelector('#iusisu_login #iusisu_password');
        iusisu_togglePassword.addEventListener('click', function (e) {
            // toggle the type attribute
            const iusisu_type = iusisu_password.getAttribute('type') === 'password' ? 'text' : 'password';
            iusisu_password.setAttribute('type', iusisu_type);
    ...
    jQuery(document).on('click','#iusisu_change-password .iusisu_form-field i',function()
        ...
        const iusisu_forgot_password = document.querySelector("#iusisu_forgot-password");
        if(iusisu_forgot_password){

            // Validate Username
            $("#iusisu_username_email-error").hide();
            let iusisu_usernameError = true;

            $("#iusisu_forgot-password").submit(function(event) {
                event.preventDefault();

                function iusisu_validateUsername() {
                    let iusisu_usernameValue = $("#iusisu_username_email").val();
                    if (iusisu_usernameValue == "") {
                        $("#iusisu_username_email-error").show();
                        iusisu_usernameError = false;
                        return false;
                    } else {
                        $("#iusisu_username_email-error").hide();
                    }
                ...
                iusisu_validateUsername();

                var iusisu_wpnonce = $("#iusisu_wpnonce").val();
                var iusisu_username_email = $("#iusisu_username_email").val();
                var ajax_url = admin_ajaxObj.ajax_url;
                var data = {
                    iusisu_username_email: iusisu_username_email,
                    iusisu_wpnonce: iusisu_wpnonce,
                    action: "iusisu_forgot_password"
                };

                if (iusisu_username_email !== "") {
                    $.ajax({
                        url: ajax_url,
                        type: "POST",
                        data: data,
                        beforeSend: function() {
                            // Show image container
                            $("#iusisu_user_loader").fadeIn(500);
                        },
                        success: function(html) {
                            $("#iusisu_username_email").css("border", "");
                            $(".iusisu_forgot-message").html(html);
                        },
                        complete: function(data) {
                            // Hide image container
                            $("#iusisu_user_loader").fadeOut(500);
                        }
                    });
                }
            });
